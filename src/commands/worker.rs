...
impl WorkerCommand {
    ...
    async fn execute_worker(&self) -> Result<()> {
        let api_config_path = self
            .args
            .api_server
            .config
            .as_deref()
            .ok_or_else(|| TlsError::ConfigError {
                message: "--worker requires --api-config so worker limits and webhook settings are explicit".to_string(),
            })?;
        let db_config_path =
            self.args
                .database
        ...
        let queue = crate::api::jobs::DatabaseJobQueue::new(
            ...
            Some(chrono::Duration::seconds(
                i64::try_from(api_config.job_retention_seconds).map_err(|_| {
