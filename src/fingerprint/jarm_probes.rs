...
/// The No13 variant is derived by filtering out 0x13xx entries.
const JARM_ALL_CIPHERS: &[[u8; 2]] = &[
    ...
    [0xc0, 0x61], [0xc0, 0x76], [0xc0, 0x77], [0xcc, 0xa8],
    [0x13, 0x05], [0x13, 0x04], [0x13, 0x03],
    [0xcc, 0x13], [0xc0, 0x11], [0x00, 0x0a], [0x00, 0x2f], [0x00, 0x3c],
    [0xc0, 0x9c], [0xc0, 0xa0], [0x00, 0x9c], [0x00, 0x35], [0x00, 0x3d],
    [0xc0, 0x9d], [0xc0, 0xa1], [0x00, 0x9d], [0x00, 0x41], [0x00, 0xba],
...
fn get_ciphers(opts: &JarmProbeOptions) -> Vec<u8> {
    let mut ciphers: Vec<[u8; 2]> = match opts.cipher_list {
        CipherList::All => JARM_ALL_CIPHERS.to_vec(),
        CipherList::No13 => JARM_ALL_CIPHERS
            .iter()
            .copied()
            .filter(|c| c[0] != 0x13)
            .collect(),
    ...
    // Flatten to bytes
    ciphers.iter().flat_map(|c| c.iter().copied()).collect()
}
