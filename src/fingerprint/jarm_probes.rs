...
fn get_extensions(opts: &JarmProbeOptions) -> Vec<u8> {
    ...
    // Supported Groups
    all_extensions.extend_from_slice(&[0x00, 0x0a, 0x00, 0x0a, 0x00, 0x08, 0x00, 0x1d, 0x00, 0x17, 0x00, 0x18, 0x00, 0x19]);
...
mod tests {
    ...
    fn test_cipher_reordering() {
        ...
        assert_eq!(reverse, vec![[0x07, 0x08], [0x05, 0x06], [0x03, 0x04], [0x01, 0x02]]);

        // Middle Out
        let middle_out = reorder_ciphers(ciphers.clone(), CipherOrder::MiddleOut);
        assert_eq!(middle_out, vec![[0x03, 0x04], [0x01, 0x02], [0x05, 0x06], [0x07, 0x08]]);
    }
