...
use crate::db::{CipherRunDatabase, ScanRecord, ProtocolRecord, CipherRecord, VulnerabilityRecord};
...
impl ChangeTracker {
    ...
    pub async fn detect_changes(&self, hostname: &str, port: u16, days: i64) -> crate::Result<Vec<ChangeEvent>> {
    ...
    pub async fn detect_changes_between(&self, scan_id_1: i64, scan_id_2: i64) -> crate::Result<Vec<ChangeEvent>> {
        ...
        let (scan1, scan2) = match (scan1_opt, scan2_opt) {
            ...
            _ => return Err(crate::TlsError::DatabaseError("One or both scans not found".to_string())),
        };

        let timestamp = scan2.scan_timestamp;

        // Detect protocol changes
        changes.append(&mut self.detect_protocol_changes(scan_id_1, scan_id_2, timestamp).await?);

        // Detect cipher changes
        changes.append(&mut self.detect_cipher_changes(scan_id_1, scan_id_2, timestamp).await?);

        // Detect certificate changes
        changes.append(&mut self.detect_certificate_changes(scan_id_1, scan_id_2, timestamp).await?);

        // Detect vulnerability changes
        changes.append(&mut self.detect_vulnerability_changes(scan_id_1, scan_id_2, timestamp).await?);

        // Detect rating changes
        changes.append(&mut self.detect_rating_changes(&scan1, &scan2, timestamp).await?);
    ...
    pub fn generate_change_report(&self, changes: &[ChangeEvent]) -> String {
        ...
        report.push_str(&format!("Change Report - {} changes detected\n", changes.len()));
        report.push_str("═══════════════════════════════════════════════════════\n\n");
    ...
    async fn detect_protocol_changes(&self, scan_id_1: i64, scan_id_2: i64, timestamp: DateTime<Utc>) -> crate::Result<Vec<ChangeEvent>> {
        ...
        let pref1 = protocols1.iter().find(|p| p.preferred).map(|p| &p.protocol_name);
        let pref2 = protocols2.iter().find(|p| p.preferred).map(|p| &p.protocol_name);
    ...
    async fn detect_cipher_changes(&self, scan_id_1: i64, scan_id_2: i64, timestamp: DateTime<Utc>) -> crate::Result<Vec<ChangeEvent>> {
        ...
        for cipher_name in set1.difference(&set2) {
            let cipher = ciphers1.iter().find(|c| &c.cipher_name == cipher_name).unwrap();
        ...
        for cipher_name in set2.difference(&set1) {
            let cipher = ciphers2.iter().find(|c| &c.cipher_name == cipher_name).unwrap();
            let severity = match cipher.strength.as_str() {
    ...
    async fn detect_certificate_changes(&self, scan_id_1: i64, scan_id_2: i64, timestamp: DateTime<Utc>) -> crate::Result<Vec<ChangeEvent>> {
    ...
    async fn detect_vulnerability_changes(&self, scan_id_1: i64, scan_id_2: i64, timestamp: DateTime<Utc>) -> crate::Result<Vec<ChangeEvent>> {
        let vulns1 = self.get_vulnerabilities(scan_id_1).await?;
        let vulns2 = self.get_vulnerabilities(scan_id_2).await?;

        let mut changes = Vec::new();

        let set1: HashSet<String> = vulns1.iter().map(|v| v.vulnerability_type.clone()).collect();
        let set2: HashSet<String> = vulns2.iter().map(|v| v.vulnerability_type.clone()).collect();

        // Resolved vulnerabilities
        for vuln_type in set1.difference(&set2) {
            let vuln = vulns1.iter().find(|v| &v.vulnerability_type == vuln_type).unwrap();
        ...
        for vuln_type in set2.difference(&set1) {
            let vuln = vulns2.iter().find(|v| &v.vulnerability_type == vuln_type).unwrap();
            let severity = match vuln.severity.as_str() {
    ...
    async fn detect_rating_changes(&self, scan1: &ScanRecord, scan2: &ScanRecord, timestamp: DateTime<Utc>) -> crate::Result<Vec<ChangeEvent>> {
        let mut changes = Vec::new();
        ...
        if scan1.overall_grade != scan2.overall_grade || scan1.overall_score != scan2.overall_score {
            ...
            changes.push(ChangeEvent {
                ...
                previous_value: scan1.overall_grade.clone().map(|g| {
                    format!("{} ({})", g, scan1.overall_score.unwrap_or(0))
                }),
                current_value: scan2.overall_grade.clone().map(|g| {
                    format!("{} ({})", g, scan2.overall_score.unwrap_or(0))
                }),
                timestamp,
            });
        }
    ...
    async fn get_leaf_certificate(&self, scan_id: i64) -> crate::Result<Option<crate::db::CertificateRecord>> {
...
mod tests {
    ...
    async fn test_generate_change_report_empty() {
        let db = Arc::new(CipherRunDatabase::new(&crate::db::DatabaseConfig::sqlite(std::path::PathBuf::from(":memory:"))).await.unwrap());
