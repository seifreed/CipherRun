...
mod target_names;
pub use target_names::{
    canonical_target, display_target_host, normalize_dns_hostname, parse_port,
    server_name_for_hostname, sni_hostname_for_target, split_target_host_port,
};
...
pub async fn connect_with_starttls(
    ...
    if let Some(protocol) = starttls {
        let negotiator =
            crate::starttls::protocols::get_negotiator(protocol, hostname.to_string(), server_mode);
...
pub async fn try_vuln_ssl_connection(
    ...
    let (hostname, use_sni) =
        openssl_hostname_and_sni(&target.hostname, config.starttls_hostname.as_deref());
