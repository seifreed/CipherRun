...
<body>
    <div class="container">
        ...
        <section>
            ...
            <table>
                <tbody>
                    <tr><th>Detected</th><td>{{cdn_detection.is_cdn}}</td></tr>
                    <tr><th>Provider</th><td>{{cdn_detection.cdn_provider}}</td></tr>
                    <tr><th>Confidence</th><td>{{cdn_detection.confidence}}</td></tr>
                    <tr><th>Indicators</th><td>{{cdn_detection.indicators}}</td></tr>
                </tbody>
            </table>
        </section>
        {{/if}}
        ...
        <section>
            ...
            <table>
                <tbody>
                    <tr><th>Detected</th><td>{{load_balancer_info.detected}}</td></tr>
                    <tr><th>Type</th><td>{{load_balancer_info.lb_type}}</td></tr>
                    <tr><th>Sticky Sessions</th><td>{{load_balancer_info.sticky_sessions}}</td></tr>
                    <tr><th>Indicators</th><td>{{load_balancer_info.indicators}}</td></tr>
                </tbody>
            </table>
        </section>
        {{/if}}
...
pub fn generate_html_report(results: &ScanResults) -> Result<String> {
    ...
    let data = json!({
        ...
        "cdn_detection": results.cdn_detection().map(|cdn| json!({
            "is_cdn": cdn.is_cdn,
            "cdn_provider": cdn.cdn_provider.as_deref().unwrap_or("N/A"),
            "confidence": format!("{:.2}", cdn.confidence),
            "indicators": if cdn.indicators.is_empty() {
                "N/A".to_string()
            } else {
                cdn.indicators.join("; ")
            },
        })),
        "load_balancer_info": results.load_balancer_info().map(|load_balancer| json!({
            "detected": load_balancer.detected,
            "lb_type": load_balancer.lb_type.as_deref().unwrap_or("N/A"),
            "sticky_sessions": load_balancer.sticky_sessions,
            "indicators": if load_balancer.indicators.is_empty() {
                "N/A".to_string()
            } else {
...
mod tests {
    ...
    fn test_html_generation_includes_fingerprint_sections() {
        ...
        let html = generate_html_report(&results).expect("test assertion should succeed");
        assert!(html.contains("<h2>CDN Detection</h2>"));
        assert!(html.contains("<h2>Load Balancer</h2>"));
        assert!(html.contains("Cloudflare"));
        assert!(html.contains("AWS ALB"));
    }
