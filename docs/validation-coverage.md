...
| Renegotiation, fallback SCSV, early data, GREASE | Negotiation/configuration | `fallback-tls`; `early-data-tls`; `grease-intolerant-tls` | `fallback-patched-tls`; `early-data-patched-tls`; `grease-tolerant-tls` | CipherRun | SCSV, early data, and GREASE have isolated handshakes; renegotiation still needs a dedicated insecure/secure pair |
