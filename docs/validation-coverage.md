...
| Heartbleed | Active heartbeat probe | `heartbleed-tls`; pinned OpenSSL 1.0.1c smoke via `make external-fixture` | `heartbleed-patched-tls` | CipherRun + OpenSSL client smoke | The external image is a real vulnerable implementation and proves legacy TLS 1.0 reachability; CipherRun confirmation remains a separate compatibility gate and is not inferred from the version string |
