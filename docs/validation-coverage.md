...
| POODLE variants, OpenSSL 0-Length, CVE-2016-2107 | Active probe | Isolated CBC oracle simulation (`poodle-variant-vulnerable-tls`); pinned OpenSSL 1.0.1c multi-version fixture covers 0-Length as `inconclusive` and CVE-2016-2107 as `inconclusive` | `poodle-variant-patched-tls`; pinned OpenSSL 1.1.1m GCM-only control | CipherRun + OpenSSL client smoke | The oracle pair validates alert asymmetry, not a vulnerable implementation; 0-Length cannot be confirmed without observing memory disclosure; CVE-2016-2107 has no reliable remote timing signal |
